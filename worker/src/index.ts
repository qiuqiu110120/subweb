// Standalone Cloudflare Worker - API backend
// This is a self-contained copy. The primary deployment target is Pages Functions.
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Return status page for non-API routes
    if (path === "/" || (!path.startsWith("/api/") && !path.startsWith("/sub/"))) {
      return new Response("ProxySubscription API is running. Frontend: Cloudflare Pages.", {
        status: 200,
